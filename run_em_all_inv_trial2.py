...
def tcp_vs_cl_model2_check(d, clb, filename):
    ...
    with open(csv_fname, 'w') as fd:
        ...
        fd.write("\nstd Values ,%.2f,%.2f"%(fit_sm,fit_sp))
        fd.write("\n")
        fd.write("\nModel y= (a*x)+sqrt(((a**2)*(x**2))+(b*x)+c) , a, b, c")
    ...
    print("\nModel2: y= (a*x)+sqrt(((a**2)*(x**2))+(b*x)+c)")
...
def tcp_vs_tr(d, filename):
    ...
    while trb>1:
      ...
      #chisq for model2:
      chisq2 = sum((d2['tcp'] - model2(d2['tr'],fit_a,fit_b,fit_c))**2/d2['tcp'])
